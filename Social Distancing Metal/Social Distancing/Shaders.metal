...
fragment half4 compositeImageFragmentShader(CompositeColorInOut in [[ stage_in ]],
                                    ...
                                    constant SharedUniforms &uniforms [[ buffer(kBufferIndexSharedUniforms) ]],
                                    constant float4 *highlightColor [[ buffer(0) ]],
                                    constant float *detectionDistance [[ buffer(1) ]])
{
    ...
    if (uniforms.useDepth) {
        ...
        if (dilatedLinearDepth < half4(*highlightColor)[3]) {
            half4 color = half4(*highlightColor);
            half4 mixedColor = mix(color, cameraColor, 0.4);
            half4 occluderResult = mix(sceneColor, mixedColor, alpha);
