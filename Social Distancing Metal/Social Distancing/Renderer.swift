...
class Renderer {
    ...
    func checkDepth(colorTexture: MTLTexture, depthTexture: MTLTexture) {
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer() {
            ...
            if let renderPassDescriptor = renderDestination.currentRenderPassDescriptor, let currentDrawable = renderDestination.currentDrawable {
                ...
                if let sceneRenderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: sceneRenderDescriptor) {
                    ...
                    sceneRenderEncoder.label = "ARSceneRenderEncoder"
                ...
                if let compositeRenderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) {
                    ...
                    compositeRenderEncoder.label = "ARCompositeRenderEncoder"
    ...
    func loadMetal() {
        ...
        capturedImagePipelineStateDescriptor.label = "ARCapturedImagePipeline"
        ...
        anchorPipelineStateDescriptor.label = "ARAnchorPipeline"
        ...
        compositePipelineStateDescriptor.label = "ARCompositePipeline"
    ...
    func compositeImagesWithEncoder(renderEncoder: MTLRenderCommandEncoder) {
        ...
        var fragmentColor = vector_float4(1.0, 0.91, 0.2, 2.1)
        var detectionDistance = 2.1
        renderEncoder.setFragmentBytes(&fragmentColor, length: MemoryLayout.size(ofValue: fragmentColor), index: 0)
        renderEncoder.setFragmentBytes(&detectionDistance, length: MemoryLayout.size(ofValue: detectionDistance), index: 1)
